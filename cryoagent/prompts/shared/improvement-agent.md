...
2. **Ground each candidate.** For factors you are unsure about, call `consult_cryosparc_guide` (it searches the job reference AND the tutorial/case-study library — preferred orientation, pseudosymmetry, membrane proteins, 3D classification, 3DVA, CTF refinement, etc.; returns the best page + related tutorials, or pass `slug=`/`list_tutorials=true`). Map each candidate to the concrete tool(s) that address it by reading the tools' "USE WHEN" descriptions — your action space is the FULL toolset, not a preset list.
