...
class PickingWorkflow:
    ...
    def _get_task_context(self, micrographs_job_uid: str) -> Dict[str, Any]:
        """Build template variables for cryosparc/particle_picking/task.md."""
        p = self.workflow_params
        diameter_max = p["diameter_max"] if p.get("diameter_max") else p["particle_diameter"] * 2.0
        ...
        return {
            ...
            "diameter_max": diameter_max,
            "box_size_pix": p["box_size_pix"],
            "num_classes": p["num_classes"],
            "lowpass_resolution": p["lowpass_resolution"],
            "select_step_title": select_step_title,
            "final_select_step_title": final_select_step_title,
            "select_params": ", ".join(select_params),
            "final_select_params": ", ".join(final_select_params),
        }
    ...
    def _create_workflow_input(self, micrographs_job_uid: str) -> str:
        ...
        return load_prompt(
            "cryosparc/particle_picking/task.md",
            self._get_task_context(micrographs_job_uid),
        )
